...
app.post("/games", async (req, res) => {
  ...
  if (!roomCode || !celebrity) {
    return res.status(400).json({ error: "roomCode and celebrity are required" });
  }
...
app.get("/rooms", async (req, res) => {
  try {
   ...
   const games = rooms.map((room: { room_code: string; turns: { full_name: string }[] }) => ({
  ...
  if (!roomCode || !username || !answer) {
    return res.status(400).json({ error: "roomCode, username, and answer are required" });
  }
  ...
  try {
    ...
    if (err.code === "P2002") {
      return res.status(409).json({ error: "That name has already been used in this game" });
    }
    console.error(err);
